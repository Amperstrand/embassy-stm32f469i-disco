...
async fn main(_spawner: embassy_executor::Spawner) {
    ...
    // Test 1: Walking 1s (full 16MB x 32 bits)
    ...
    // Test 2: Walking 0s (full 16MB x 32 bits)
    ...
    // Test 11: Multi-pass random (3 passes, full 16MB)
    ...
    // Test 12: Byte-level (first 64K)
    defmt::info!("TEST byte_level: RUNNING");
    {
        let mut ok = true;
        let ram_bytes: &mut [u8] =
            unsafe { core::slice::from_raw_parts_mut(base as *mut u8, words * 4) };
    ...
    // Test 13: Halfword-level (first 32K)
