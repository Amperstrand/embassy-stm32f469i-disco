...
static mut RESULTS: [(&'static str, bool); MAX_TESTS] = [(&"", false); MAX_TESTS];
...
fn draw_u32(fb: &mut embassy_stm32f469i_disco::FramebufferView<'_>, x: i32, y: i32, style: &MonoTextStyle<Rgb565>, val: u32) {
    let mut buf = [0u8; 12];
    let mut i = buf.len();
    let mut v = val;
    loop {
...
fn draw_result(fb: &mut embassy_stm32f469i_disco::FramebufferView<'_>, y: &mut i32, name: &str, passed: bool) {
...
fn draw_status_line(fb: &mut embassy_stm32f469i_disco::FramebufferView<'_>, y: &mut i32, passed: usize, failed: usize, running: &str) {
    let ps = make_style();
...
fn draw_summary(fb: &mut embassy_stm32f469i_disco::FramebufferView<'_>, y: i32, passed: usize, failed: usize) {
    ...
    let hs = make_header_style();
    draw_text(fb, "STM32F469I-DISCO Hardware Diagnostics v0.2.0", 8, sy + 6, &hs);
}
...
async fn main(_spawner: embassy_executor::Spawner) {
    ...
    // --- GPIO ---
    unsafe { tpass_fn("GPIO Input PA0", || {
    ...
    unsafe { tpass_fn("RNG Not Zeros", || {
        ...
        loop {
            ...
            if sr.seis() | sr.ceis() {
                ...
                rng.sr().modify(|w| { w.set_seis(false); w.set_ceis(false); });
    ...
    unsafe { tpass_fn("RNG Uniqueness", || {
        ...
        while i < 64 {
            ...
            let mut j = 0;
            while j < i {
                if buf[j] == buf[i] { is_unique = false; break; }
                j += 1;
            }
    ...
    unsafe { tpass_fn("ADC Temp Sensor", || {
        ...
        adc.cr2().modify(|w| { w.set_adon(false); w.set_cont(false); });
        adc.cr1().modify(|w| { w.set_res(stm32_metapac::adc::vals::Res::BITS12); w.set_scan(false); });
        adc.sqr1().write(|w| { w.set_l(0); w.set_sq(0, 0); });
        adc.sqr3().write(|w| { w.set_sq(0, 18); });
        adc.smpr2().write(|w| { w.set_smp(18, stm32_metapac::adc::vals::SampleTime::CYCLES480); });
    ...
    unsafe { tpass_fn("ADC VREFINT", || {
        cortex_m::asm::delay(10_000);
        let adc = stm32_metapac::ADC1;
        adc.sqr3().write(|w| { w.set_sq(0, 17); });
        adc.smpr2().write(|w| { w.set_smp(17, stm32_metapac::adc::vals::SampleTime::CYCLES480); });
        adc.cr2().modify(|w| w.set_swstart(true));
        while !adc.sr().read().eoc() {}
        let sample = adc.dr().read().0 as u16;
    ...
    for i in 0..p1_total {
        let (name, passed) = unsafe { RESULTS[i] };
    ...
    unsafe { tpass_fn("SDRAM Boundary Spots", || {
        let mut ok = true;
        for r in 0u32..16 {
    ...
    unsafe { tpass_fn("SDRAM End-of-RAM", || {
        let last = 16384usize;
        ...
        for word in ram[start..].iter() {
            ...
            seed ^= seed >> 17;
            seed ^= seed << 5;
            if *word != seed { return false; }
        }
        true
    }) };

    unsafe { tpass_fn("SDRAM Byte/Halfword", || {
        let mut ok = true;
    ...
    unsafe { tpass_fn("DMA 64B Transfer", || {
        ...
        dma2.st(0).cr().write(|w| {
            w.set_en(false); w.set_dir(vals::Dir::MEMORY_TO_MEMORY);
            w.set_circ(false); w.set_pinc(true); w.set_minc(true);
            w.set_psize(vals::Size::BITS8); w.set_msize(vals::Size::BITS8);
            w.set_pl(vals::Pl::VERY_HIGH);
        });
        dma2.st(0).fcr().write(|w| { w.set_dmdis(vals::Dmdis::ENABLED); w.set_fth(vals::Fth::FULL); });
        ...
        dma2.ifcr(0).write(|w| { w.set_tcif(0, true); w.set_htif(0, true); w.set_feif(0, true); w.set_dmeif(0, true); w.set_teif(0, true); });
        true
    }) };

    unsafe { tpass_fn("DMA 4096B Transfer", || {
        use stm32_metapac::dma::vals;
        let dma2 = stm32_metapac::DMA2;
        dma2.st(0).cr().write(|w| {
            w.set_en(false); w.set_dir(vals::Dir::MEMORY_TO_MEMORY);
    ...
    unsafe { tpass_fn("DMA 1024B Transfer", || {
        use stm32_metapac::dma::vals;
        let dma2 = stm32_metapac::DMA2;
        dma2.st(0).cr().write(|w| {
            w.set_en(false); w.set_dir(vals::Dir::MEMORY_TO_MEMORY);
    ...
    unsafe { tpass_fn("DMA Repeated 10x", || {
        use stm32_metapac::dma::vals;
        let dma2 = stm32_metapac::DMA2;
        for _ in 0..10u32 {
            dma2.st(0).cr().write(|w| {
    ...
    defmt::info!("Entering touch demo...");
    {
        ...
        Rectangle::new(Point::new(8, 28), Size::new(464, 1))
            ...
            .draw(&mut fb)
            .ok();
        draw_text(&mut fb, "Touch the screen. Coordinates shown below.", 8, 40, &ts);
